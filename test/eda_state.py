...
def list_db(path, fields, coalesce=True, eql=None):
    if DEBUG:
        print(f'Got list_db request for path: {path}, fields: {fields}, coalesce: {coalesce}')
    global gStateDb
    result = []
...
def get_scratchpad_data(key):
    if DEBUG:
        print(f'Got scratchpad_list request for key: {key}')
    global scratchpad_list
    result = {}
